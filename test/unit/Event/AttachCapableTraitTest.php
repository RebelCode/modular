...
use RebelCode\Modular\Events\AttachCapableTrait as TestSubject;
 ...
 * @since [*next-version*]
 */
class AttachCapableTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'RebelCode\Modular\Events\AttachCapableTrait';
    ...
    /**
     * Tests the `_attach()` method to assert whether an exception is thrown when the event manager is null.
     *
     * @since [*next-version*]
     */
    public function testAttachNullEventManager()
    {
        $subject = $this->createInstance();
        $reflect = $this->reflect($subject);
